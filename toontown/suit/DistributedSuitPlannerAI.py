...
class DistributedSuitPlannerAI(DistributedObjectAI.DistributedObjectAI, SuitPlannerBase.SuitPlannerBase):
    ...
    def __init__(self, air, zoneId):
        ...
        #custom cog levels for COG HQ's
        self.cogHQZones = [11000, 11200, 12000, 13000, 10000]
    ...
    def pickLevelTypeAndTrack(self, level=None, type=None, track=None):
        ...
        if type == None:
            if self.zoneId in self.cogHQZones:
                minusVar = 6
            else:
                minusVar = 4
            typeChoices = list(range(max(level - minusVar, 1), min(level, self.MAX_SUIT_TYPES) + 1))
